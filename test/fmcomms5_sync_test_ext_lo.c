...
int main(void)
{
    ...
    // ADF5355 set up
    struct iio_device *dev_lo = iio_context_find_device(ctx, "adf5355");
    struct iio_channel *chan_lo = iio_device_find_channel(dev_lo, "altvoltage0", true);

    // Enable external lo
    double estD = 0;
    check_phase_sma(ctx, &estD);
    iio_device_debug_attr_write_longlong(dev_phy,"adi,external-rx-lo-enable", 1);
    iio_device_debug_attr_write_longlong(dev_phy,"adi,external-tx-lo-enable", 1);
    iio_device_debug_attr_write_longlong(dev_phy_slave,"adi,external-rx-lo-enable", 1);
    iio_device_debug_attr_write_longlong(dev_phy_slave,"adi,external-tx-lo-enable", 1);
    // Test sync
    long long freq = 0;
    int freqGHZ, ret=0;
    for (freqGHZ = 19; freqGHZ<20; freqGHZ++) {
        ...
        freq = 100000000*(long long)freqGHZ;

        ret = iio_channel_attr_write_longlong(chan_lo, "frequency", freq);
    ...
    double est = 0, est2 = 0;
    for (freqGHZ = 10; freqGHZ<400; freqGHZ = freqGHZ + 5) {

        printf("#### Calibrating FMComms5 at LO %f GHz ####\n",(double)freqGHZ/100);
        freq = 20000000*(long long)freqGHZ;
        printf("%lld\n",freq);
        for (k = 0; k<runs_per_freq; k++) {
          ...
          ret = iio_channel_attr_write_longlong(chan_lo, "frequency", freq);
          ad9361_sleep_ms();
          ad9361_sleep_ms();ad9361_sleep_ms();ad9361_sleep_ms();
